...
func RunMixedBenchmark(params BenchmarkParams, configFilePath string) {
	start := time.Now() // Start the timer before running benchmarks

	// Adjust the proportions of PUT, GET, and DELETE as needed
	RunPutBenchmark(params, configFilePath)
	RunGetBenchmark(params, configFilePath)
	RunDeleteBenchmark(params, configFilePath)
